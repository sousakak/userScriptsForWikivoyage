mw.loader.using([
    ...
    const vCardConfig = require( 'ext.gadget.getVCardSetting' ) || window.voy.VCardSetting;
    const Config = require('./visibleCard/config.json');
    CATEGORIES = [
        {
            value: 'access',
            label: 'Access',
    ...
    const dialog = vCard => {
        return {
            template: `
                ...
                >
                    <template #header>
                        <h2 class="voy-vCard-dialog__title">visibleCard</h2>
                        <cdx-button
                            action="default"
                            weight="quiet"
                            @click="open = false"
